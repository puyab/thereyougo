<nav class="w-full h-max bg-[#D29A9A80] flex items-center justify-between px-6 py-4 lg:py-9 lg:px-24">
  <Link href="{{route('home')}}">
  <figure>
    <img src="{{asset('images/logo.png')}}" alt="Logo" class="w-full h-max max-w-[86.73px] aspect-auto"/>
  </figure>
  </Link>
  <div class="w-max h-max flex items-center justify-center gap-7 lg:gap-14">
    ...
    @else
      <x-link class="font-bold text-xs sm:text-base text-black hidden md:block font-inter">The Dream Office</x-link>
      <x-link :href="route('login')" class="font-bold text-xs sm:text-base text-black font-inter">Login</x-link>
      <x-link :href="route('register')">
        <x-button class="text-xs md:text-base py-1.5 px-2 md:py-3.5 md:px-7 md:rounded-full w-max rounded-md">Apply
          now
        </x-button>
      </x-link>
    @endif
