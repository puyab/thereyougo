...
@endphp
<Swiper class="w-full mx-auto max-w-[1920px]" :spaceBetween="24" :loop="true" :autoplay="false"
        v-bind:modules="SwiperModules"
            ...
            <div class="w-[187px] lg:w-[375px] relative pb-[48p] flex flex-col gap-1">
                ...
                <div class="w-full h-max flex flex-row gap-2 items-start justify-between lg:px-6">
                    <figure
                        ...
                        <img class="w-full h-full absolute inset-0"
                             src="{{$avatar !== '' ? $avatar : asset('images/not-found.jpg')}}"
                             alt="{{$profile->first_name}} {{$profile->last_name}}"/>
                    </figure>
                    <div class="w-full flex flex-col items-start justify-start">
                        <span class="font-light text-xs sm:text-sm md:text-xl text-[#292D32]">{{$profile->first_name}} {{$profile->last_name}}.</span>
                        <span class="w-max max-w-[138px] lg:max-w-[210px] text-xs sm:text-sm md:text-base text-[#292D32]">{{$profile->role}}, {{$profile->company}}</span>
